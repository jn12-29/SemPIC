...
Both KL and CE training cache teacher answer token sequences only. For KL, each request is scored online with a teacher-forced forward over the current full prompt and its cached answer tokens, including EOS. The teacher runs in eval mode with LoRA disabled and no gradients, then restores the student training state. This avoids repeated autoregressive decoding and persistent sequence-length × vocabulary logits storage, at the cost of a teacher forward per request on each training pass. CE uses the tokens directly without a teacher forward. Generation-cache artifacts contain sequences and text only; training never writes computed logits back to the cache.

Training never writes a configured cache artifact. It validates coverage from `manifest.json` without loading tensor payloads, and loads only each requested sample from `cache.safetensors`. The artifact teacher, tokenizer path and current tokenizer state (`padding_side`, pad ID, and EOS ID), and dtype must match training. When `cache_path` is a list, training composes the union for mixture training. Their decoding limits may differ. Identical duplicate entries are accepted, conflicting duplicates fail. `resolved_config.json` remains provenance rather than a training-time gate. Cache lookup uses raw semantic sample identity rather than token IDs, templates, tokenizer settings, or generation settings. See [generation_cache_config.md](./generation_cache_config.md) for artifact generation.
