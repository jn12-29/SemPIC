...
def bio_ret_eval_generator(
    ...
    **kwargs
    ...
    def get_question_answer(index: int) -> tuple[str, str, str, str]:
        item = ds['test'][index]
        rand_key = rng.choice(BIO_ATTRIBUTES)
        if question_type == "completion":
            semantic_question = rng.choice(item[rand_key]).rstrip()
            question = f"## {semantic_question}"
        elif question_type == "QA":
            name = item['name']
            question = format_question(rand_key, name, rng)
            semantic_question = question.removesuffix(
                " Respond with the answer only. Answer:"
            )
        else:
            raise ValueError(f"Unknown question_type: {question_type}")

        answer = item['labels'][rand_key]
        return question, semantic_question, answer, rand_key
    ...
    for fs_index in few_shot_indices:
        ...
        fs_question, fs_semantic_question, fs_answer, fs_attribute = get_question_answer(fs_index)
        few_shot_strs.append(f"{fs_data_str} {fs_question} {fs_answer}.")
        semantic_shots.append(SemanticShot(
            documents=[fs_data_str],
            query=fs_semantic_question,
            answer=fs_answer,
            task={"attribute": fs_attribute},
        ))
    ...
    for i in range(num_samples):
        idx = all_indices[i]
        question_str, semantic_question, answer_str, attribute = get_question_answer(idx)
...
def filter_bio_dataset(
    ...
    LOGGER.info("Filtered dataset from %d to %d items", len(train_split_pd), len(indices_to_keep))
