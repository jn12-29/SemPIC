...
def get_ret_eval_generator(
    ...
    Args:
        ...
        template (str|None): The name of the chat template function to apply to each typed prompt. Should be one of the keys in TEMPLATE_FUNC_DICT.
        ...
        tokenizer (Any|None): Tokenizer object passed to template functions that need the model chat template.
    """
    ...
    def wrapped_eval_generator() -> Iterator[RetEvalEntry]:
        extra_kwargs = {"tokenizer": tokenizer} if name == "niah" else {}
        for eval_entry in eval_generator(
            ...
            **extra_kwargs,
            ...
            yield template_func(eval_entry, tokenizer=tokenizer, **(template_kwargs or {}))
