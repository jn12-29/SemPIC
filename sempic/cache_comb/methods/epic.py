...
def _epic_context_indices(
    ...
    for span in prompt.parts:
        if span.kind == "context":
            indices.extend(
                range(span.start, min(span.end, span.start + recompute_tokens))
            )
    return indices
...
def epic_eval(
    ...
    if kwargs is not None and kwargs != {}:
        LOGGER.warning("epic_eval got unexpected kwargs: %s", kwargs)

    inputs = prepare_recompute_inputs(
        "epic", model, prompt, prepared_kvs
    )
    ...
    selected_context_indices = _epic_context_indices(prompt, recompute_tokens)
