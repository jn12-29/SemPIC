...
def llama_recompute_kv(
    model: LlamaForCausalLM|LlamaModel,
    kv_cache: KVCache,
    ...
    assert isinstance(decoder_layer, LlamaDecoderLayer), "Layer is not a LlamaDecoderLayer"
    ...
    attn: LlamaAttention = decoder_layer.self_attn
    ...
    query_states, key_states_from_hs = llama_apply_rotary_pos_emb(
    ...
    if attention_basis_sink is not None:
        ...
        if attn_implementation == "eager":
            attention_interface = llama_eager_attention_forward
        else:
